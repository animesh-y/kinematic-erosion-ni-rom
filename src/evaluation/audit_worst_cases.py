...
class MultiChannelCNN(nn.Module):
    def __init__(self, in_channels=31, latent_dim=517):
        super().__init__()
        act = nn.GELU
        self.in_channels = in_channels
        self.encoder = nn.Sequential(
            SemiCircularConv2d(in_channels, 32, 3, 1), nn.BatchNorm2d(32), act(), nn.MaxPool2d(2, 2),
            SemiCircularConv2d(32, 64, 3, 1), nn.BatchNorm2d(64), act(), nn.MaxPool2d(2, 2),
            SemiCircularConv2d(64, 128, 3, 1), nn.BatchNorm2d(128), act(), nn.MaxPool2d(2, 2),
            SemiCircularConv2d(128, 256, 3, 1), nn.BatchNorm2d(256), act(), nn.MaxPool2d(2, 2),
            nn.AdaptiveAvgPool2d((1, 1))
        )
        self.flatten_size = 256
        self.fc_encode = nn.Linear(self.flatten_size, latent_dim)
        self.fc_decode = nn.Linear(latent_dim, 256 * 32 * 16)
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(256, 128, 2, 2), nn.BatchNorm2d(128), act(),
            nn.ConvTranspose2d(128, 64, 2, 2), nn.BatchNorm2d(64), act(),
            nn.ConvTranspose2d(64, 32, 2, 2), nn.BatchNorm2d(32), act(),
...
def main():
    ...
    train_indices, test_idx = train_test_split(np.arange(N), test_size=0.20, random_state=42, shuffle=True)
    ...
    for block_id, b_info in sorted(blocks_config.items()):
        ...
        model_path = fos.path.join(_REPO_ROOT, 'block_pod_powerlaw_study/models/freq_weighted_cnn_block_{block_id}_fold_0.pt')
        if os.path.exists(model_path):
            cnn.load_state_dict(torch.load(model_path, map_location=device, weights_only=True))
        # Load standard POD
        Phi = np.load(fos.path.join(_REPO_ROOT, 'block_pod_powerlaw_study/models/standard_pod_basis_{block_id}_fold_0.npy'))
        Y_flat = Y_power.reshape(N_all, -1)
        A = Y_flat @ Phi.T
        Y_recon = A[cache_indices] @ Phi
        Y_recon = Y_recon.reshape(len(test_idx), len(variables), 128622)
    ...
    X_raw_all = np.zeros((N, 24, 582, 221), dtype=np.float32)
    ...
    M_factors_all = np.max(np.abs(Y_power.reshape(N, 24, -1)), axis=-1) + 1e-8
    Z_norm_all = Y_power / M_factors_all[:, :, None, None]
    
    all_cnn = MultiChannelCNN(in_channels=24, latent_dim=517).to(device)
    ...
    methods = ['Actual', 'POD', 'HOSVD', 'Block-CNN', 'All-CNN', 'Composite', 'GPR', 'ANN']
    predictive_methods = ['POD', 'HOSVD', 'Block-CNN', 'All-CNN', 'Composite', 'GPR', 'ANN']
    gpr_ann_data = np.load(os.path.join(_REPO_ROOT, 'block_pod_powerlaw_study/test_set_predictions.npy'), allow_pickle=True).item()
    predictions_all["GPR"] = np.zeros_like(predictions_all["POD"])
    predictions_all["ANN"] = np.zeros_like(predictions_all["POD"])
    for v_i, v_name in enumerate(all_vars):
        if v_name in gpr_ann_data["gpr"]:
            predictions_all["GPR"][:, v_i] = gpr_ann_data["gpr"][v_name]
            predictions_all["ANN"][:, v_i] = gpr_ann_data["ann"][v_name]
    ...
    for emodel in erosion_models:
        ...
        for m in predictive_methods:
            ...
            for met in ['mse', 'r2', 'fw_mse', 'fw_r2', 'ssim', 'psnr']:
                ...
                # Extract worst case parameters
                row = df_sorted.loc[worst_case]
                worst_stokes = stokes[case_names.index(worst_case)]
                logger.info(f"  {met.upper()}: Avg={avg:.4e} | Best={best:.4e} | Worst={worst:.4e} (Case: {worst_case} | U={row['velocity']} m/s, rho={row['density']}, d={row['size']} um, R/D={row['bend_ratio']}, St={worst_stokes:.2f})")

    # Evaluate oka_erosion_norm
    oka_idx = all_vars.index('oka_erosion_norm')
    logger.info(f"============== FINAL RESULTS FOR OKA_EROSION_NORM (Fold 0, St > 1) ==============")
    oka_metrics = {m: {'mse':[], 'r2':[], 'fw_mse':[], 'fw_r2':[], 'ssim':[], 'psnr':[]} for m in predictive_methods}
    for idx_case, c_name in enumerate(test_cases):
        actual_map = predictions_all['Actual'][idx_case, oka_idx]
        vmax = np.max(actual_map)
        if vmax == 0: vmax = 1e-8
        for method in predictive_methods:
            pred_map = predictions_all[method][idx_case, oka_idx]
            mse, r2, fw_mse, fw_r2, s, p = calc_metrics(actual_map, pred_map, vmax)
            oka_metrics[method]['mse'].append(mse)
            oka_metrics[method]['r2'].append(r2)
            oka_metrics[method]['fw_mse'].append(fw_mse)
            oka_metrics[method]['fw_r2'].append(fw_r2)
            oka_metrics[method]['ssim'].append(s)
            oka_metrics[method]['psnr'].append(p)
            
    for m in predictive_methods:
        logger.info(f"Method: {m}")
        for met in ['mse', 'r2', 'fw_mse', 'fw_r2', 'ssim', 'psnr']:
            vals = oka_metrics[m][met]
            if len(vals) == 0: continue
            best = np.min(vals) if met in ['mse', 'fw_mse'] else np.max(vals)
            worst = np.max(vals) if met in ['mse', 'fw_mse'] else np.min(vals)
            worst_idx = np.argmax(vals) if met in ['mse', 'fw_mse'] else np.argmin(vals)
